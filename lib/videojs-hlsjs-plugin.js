(function (window, videojs, Hlsjs, document) {
    ...
    var registerSourceHandler = function (videojs) {
        ...
        function Html5Hlsjs(source, tech) {
            ...
            function switchQuality(qualityId) {
                ...
                var isSmoothSwitching = false;
                if (tech.options_.hlsjsConfig.hasOwnProperty('bitrateSmoothSwitching')) {
                    if (tech.options_.hlsjsConfig.bitrateSmoothSwitching === true) {
                        isSmoothSwitching = true;
                    }
                }
            ...
            function _onLevelSwitchingEventHandler(event, data) {
                tech.trigger('levelSwitching', data);
            }

            function _onLevelSwitchedEventHandler(event, data) {
                tech.trigger('levelSwitched', data);
            }

            function _onLevelLoadedEventHandler(event, data) {
                tech.trigger('levelLoaded', data);
            }
                ...
                if (_metadata) {
                    ...
                    if (_metadata.levels.length > 1) {
                        var autoLevel = {
                            ...
                            selected: tech.options_.hlsjsConfig.startLevel === -1
                        };
                    ...
                    _metadata.levels.forEach(function (level, index) {
                        ...
                        quality.selected = (index === tech.options_.hlsjsConfig.startLevel);
                    ...
                    // config로 넘어온 startLevel값이 data.levels의 갯수보다 클 경우 data.levels의 배열길이 값으로 설정한다.
                    var targetStartLevel = -1;
                    if (typeof tech.options_.hlsjsConfig.startLevel !== 'undefined') {
                        targetStartLevel = (tech.options_.hlsjsConfig.startLevel > _metadata.levels.length - 1) ? (_metadata.levels.length - 1) : tech.options_.hlsjsConfig.startLevel;
                    }
                    _hls.startLevel = targetStartLevel;
                    _hls.loadLevel = _hls.startLevel;
                    _hls.startLoad();
            ...
            function _initHlsjs() {
                ...
                _hls.on(Hlsjs.Events.FRAG_CHANGED, _onFragChangeEventHandler);
                _hls.on(Hlsjs.Events.LEVEL_SWITCHING, _onLevelSwitchingEventHandler);
                _hls.on(Hlsjs.Events.LEVEL_SWITCHED, _onLevelSwitchedEventHandler);
                _hls.on(Hlsjs.Events.LEVEL_LOADED, _onLevelLoadedEventHandler);
            ...
            _video.addEventListener('error', function (evt) {
                ...
                // kollus-live에서 currentTarget이 null인 상태에 에러가 발생하여 예외처리 (2020/03/27)
