...
var attachVideojsStreamrootProvider = function (window, videojs, Hls) {
    function StreamrootProviderHLS (source, tech) {
        ...
        function switchQuality(qualityId, trackType) {
            ...
            if (tech.options_.hlsjsConfig.hasOwnProperty('bitrateSmoothSwitching')) {
                if (tech.options_.hlsjsConfig.bitrateSmoothSwitching === true) {
                    is_smooth_switching = true;
                }
            }
            if (is_smooth_switching) {
        ...
        function _onError(event, data) {
            ...
            console.error('[' + data.type + '][' + data.fatal + ']' + _errorCounts[data.type]);
            ...
            //     //     console.info("level load retry count : " + _errorCounts[data.type]);
            //     //     if (_errorCounts[data.type] > _levelLoadMaxRetry) {
            //     //         // trigger fatal network error in player
            //     //         _hls.destroy();
            //     //         tech.error = function() { return error; };
            //     //         tech.trigger("error");
            //     //     }
            //     // }
            //     console.info("non fatal error retry count : " + _errorCounts[data.type]);
            //     if (_errorCounts[data.type] > _nonFatalErrorMaxRetry) {
            //         // trigger fatal network error in player
            //         _hls.destroy();
            //         tech.error = function() { return error; };
            //         tech.trigger("error");
        ...
        function _onMetaData(event, data) {
            ...
            if (data.levels.length > 1) {
                var autoLevel = {
                    id: -1,
                    label: "auto",
                    selected: -1 === tech.options_.hlsjsConfig.startLevel
                };
            ...
            if (tech.options_.hlsjsConfig.startLevel !== null) {
                var target_start_level = (tech.options_.hlsjsConfig.startLevel > data.levels.length - 1) ? (data.levels.length - 1) : tech.options_.hlsjsConfig.startLevel;
                _hls.startLevel = target_start_level;
